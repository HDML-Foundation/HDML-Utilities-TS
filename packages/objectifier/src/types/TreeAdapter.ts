...
/**
 * Tree adapter is a set of utility functions that provides minimal
 * required abstraction layer beetween parser and a specific AST
 * format. Note that `TreeAdapter` is not designed to be a general
 * purpose AST manipulation library. You can build such library on top
 * of existing `TreeAdapter` or use one of the existing libraries from
 * npm.
 *
...
> {
   ...
   * @param attrs - Attribute name-value pair array. Foreign
   * attributes may contain `namespace` and `prefix` fields as well.
   */
  createElement(
    tagName: string,
    namespaceURI: NS,
  ...
  /**
   * Returns the given element's attributes in an array, in the form
   * of name-value pairs. Foreign attributes may contain `namespace`
   * and `prefix` fields as well.
   *
  ...
  /**
   * Inserts a child node to the given parent node before the given
   * reference node.
   *
   * @param parentNode - Parent node.
   * @param newNode -  Child node.
  ...
  /**
   * Inserts text into a node. If the last child of the node is a text
   * node, the provided text will be appended to the text node
   * content. Otherwise, inserts a new text node with the given text.
   *
  ...
  /**
   * Inserts text into a sibling node that goes before the reference
   * node. If this sibling node is the text node, the provided text
   * will be appended to the text node content. Otherwise, inserts a
   * new sibling text node with the given text before the reference
   * node.
   *
   * @param parentNode - Node to insert text into.
   * @param text - Text to insert.
  ...
  /**
   * Sets the document type. If the `document` already contains a
   * document type node, the `name`, `publicId` and `systemId`
   * properties of this node will be updated with the provided values.
   * Otherwise, creates a new document type node with the given
   * properties and inserts it into the `document`.
   *
   * @param document - Document node.
  ...
  /**
   * Optional callback for elements being pushed to the stack of open
   * elements.
   *
   * @param element The element being pushed to the stack of open
   * elements.
   */
  onItemPush?: (item: T["element"]) => void;

  /**
   * Optional callback for elements being popped from the stack of
   * open elements.
   *
